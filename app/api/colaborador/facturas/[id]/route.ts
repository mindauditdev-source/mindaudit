...
export async function GET(
  ...
  try {
    ...
    if (compra.colaboradorId !== (session.user as any).id) {
    ...
    if (!checkoutSession.invoice) {
        ...
        return NextResponse.json({ error: "La factura aún se está procesando en Stripe. Por favor, intenta en unos momentos." }, { status: 404 });
    }
